  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<ProductModel>>(
        future: controller.fetchProductsWithStockStatus(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(TColors.blue)));
              ...
              child: Column(
                ...
                children: [
                  TAnimationLoaderWidget(
                    text: '',
                    animation: TImages.saleOrderAnimation,
                    showAction: false,
                    actionText: '',
                  ),
                  Text(S.of(context).noProductsStock),
                ],
              ),
            );
          } else {
