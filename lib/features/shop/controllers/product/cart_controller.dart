...
import 'package:easyshoppin_eshop/features/shop/models/cart_item_model.dart';
...
import '../../../../common/widgets/products/product_cards/dialogs/remove_product_dialog.dart';
...
class CartController extends GetxController {
  ...
  void addToCart(BuildContext context, ProductModel product) {
    ...
    if (product.productType == ProductType.variable.toString()) {
      if (variationController.selectedVariation.value.stock < 1) {
        TLoaders.warningSnackBar(message: 'Selected variation is out stock.', title: 'Oh Snap!');
        return;
      }
    } else {
      if (product.stock < 1) {
  ...
  void removeOneFromCartWithDialog(CartItemModel item) {
    int index = cartItems.indexWhere((cartItem) => cartItem.productId == item.productId && cartItem.variationId == item.variationId);
